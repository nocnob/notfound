...
Q: curl: (60) SSL certificate problem: unable to get local issuer certificate

A: 自签名证书无法通过网络获取证书链，因此在配置 Nginx 时，需要将证书链上除根证书以外的所有证书放到同一个证书文件。

这里需要将 `app1.demo.com/app1.demo.com.crt` 和 `ca2.demo.com/cacert.pem` 放到一起。
...
server {
  ...
  ssl_certificate app1.demo.com.crt;
  ssl_certificate_key app1.demo.com.key;
  # ...
}
----

配置多级证书时，需要将中间证书也添加到证书文件，在这里需要将 `app1.demo.com/app1.demo.com.crt`、`ca2.demo.com/cacert.pem` 两个文件中的 `-----BEGIN CERTIFICATE-----` 和 `-----END CERTIFICATE-----` 部分放到同一个文件，根证书可选。
