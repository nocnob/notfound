...
sudo cp /etc/docker/certs.d/mirror.registry.test/cacert.pem \
  /data/containerd/certs.d/docker.io/mirror.registry.test.crt
----
* 复制 CA 证书
...
[host."https://mirror.registry.test"]
  ca = "/etc/containerd/certs.d/docker.io/mirror.registry.test.crt"
----
* ca 为容器内的目录
...
# 另一个终端查看集群日志
docker logs -f mirror.registry.test
