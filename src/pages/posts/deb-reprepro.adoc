...
[source,bash]
----
sudo htpasswd /etc/nginx/.htpasswd packages
sudo htpasswd /etc/nginx/.htpasswd jenkins
----
...
可以在 Nginx 服务端保存一份，方便部署时使用，可以考虑保存到：`/var/www/packages/incoming/debian/dput.cf.txt`。
